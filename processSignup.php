...
//combine names, encrypt password, and make sure facebook includes http://
	...
	{
		$_SESSION['DM2-status'] = $_SESSION['DM2-status'] . "<p>Your first and last name cannot be left blank.</p>";
	}

	if (strlen($username) == 0)
	{
		$_SESSION['DM2-status'] = $_SESSION['DM2-status'] . "<p>Your username cannot be left blank.</p>";
	}

	if (strlen($password) == 0)
	{
		$_SESSION['DM2-status'] = $_SESSION['DM2-status'] . "<p>Your password cannot be left blank.</p>";
	}

	if (strlen($facebook) == 0)
	{
		$_SESSION['DM2-status'] = $_SESSION['DM2-status'] . "<p>Your Facebook link cannot be left blank.</p>";
	}
	else if (strlen($facebook) < 14)
	{
		$_SESSION['DM2-status'] = "<p>Please enter your Facebook in the format http://www.facebook.com/<i>yourusernamehere</i></p>";
	}
	...
	if (strlen($email) == 0)
	{
		$_SESSION['DM2-status'] = $_SESSION['DM2-status'] . "<p>Your email address cannot be left blank.</p>";
	}
...
if ((mysql_num_rows($sql0)==0) && (strlen($_SESSION['DM2-status'])==0))
{
...
P.S. Super awesome stuff at http://www.twitter.com/DMAssassins";
	...
	mail($email, $subject, $message, $headers);
	 ...
	 $_SESSION['DM2-username'] = htmlspecialchars($username);
	 $_SESSION['DM2-usertype'] = 0;

	echo('<SCRIPT LANGUAGE="JavaScript">redirURL = "index.php";self.location.href = redirURL;</script>');
}
else
{
	$_SESSION['DM2-status'] = "<p>An error occurred while processing your account. Please try again later.</p>";
}
}
...
{//if user already has an account, inform them
	...
	{
		$_SESSION['DM2-status'] = "<p>That username is already taken.</p>";
	}
	else if ((strlen($_SESSION['DM2-status'])==0)|| (mysql_num_rows($sql0)!=0))
		$_SESSION['DM2-status'] = "<p><strong>Error:</strong> You have already signed up. If you believe this is in error please contact your overall.<p>";
